...
### コンテンツ
コンテンツとは実際に画面に表示されるテキストや、画像のことです。
...
以下の例で使用してる`imgタグ`は画像を表示するタグで、  
終了タグがありません。

```html
<!-- ②のタグの例 -->
<img src="ここに画像のURL" alt="" style="">
```

それぞれのタグごとに①の使い方と②の使い方どちら決まっており、  
...
### 要素
**コンテンツ**とは、実際に画面に表示されるテキストのこと、  
**タグ**とはコンテンツを囲っている部分で、コンテンツは含みませんが、
**要素**は開始タグから、終了タグまでコンテンツも含んだ全体になります。
...
## HTMLの基本構成
HTMLで使用されるタグ(要素)を紹介したので、  
次はそのタグ(要素)を使用してどのようにWebサイトを作成するか説明します。
...
`alt`属性にはなんの画像かを書きます。画像が表示できない場合に画面に表示されます。  

```html
 <div>
     <img src="http://nexseed.net/images/logo.jpg" alt="NexSeed">
 </div>
```
